use crate::chess_engine::{Board, Position, PieceType};
use std::fmt::Display;
...
mod chess_engine {
    ...
    fn test_game_states() {
        ...
        assert_eq!(board3.get_game_state(), GameState::Check(Color::White));
    }
    ...
    impl Piece {
                    ...
                    match self.position.position_at_offset(&(dir.0 * distance, dir.1 * distance)) {
                        Some(position_moving_to) => {
                            ...
                            if moving {
                                ...
                                for piece in &board.pieces {
                                    if piece.color == self.color {} else {
                                        if piece.can_check(king_position.clone(), board) {
        ...
        fn can_move_to(&self, position: Position, board: &Board) -> bool {
            let available_moves = self.get_possible_moves(board, true);
            for available_move in &available_moves {
                if available_move.end_position == position {
                    return true;
                }
    ...
    impl Board {
        ...
        fn get_pieces_as_borrow(&self) -> Vec<&Piece> {
            let mut pieces_as_borrow = Vec::new();
            for piece in &self.pieces {
                &pieces_as_borrow.push(piece);
            }
            return pieces_as_borrow;
        }

        fn get_pieces_as_borrow_mut(&mut self) -> Vec<&Piece> {
            let mut pieces_as_borrow = Vec::new();
            for piece in &self.pieces {
                &pieces_as_borrow.push(piece);
            }
...
fn main() {
    ...
    let moves = board.get_current_player_moves();
    let mut board_repr: [[Option<PieceType>; 8]; 8] = [
    ...
    board.take_move("h2-h4".to_string());
    board.take_move("h7-h5".to_string());
    board.take_move("h1-h3".to_string());

    for mov in board.get_current_player_moves() {
        if mov.1.get(0).is_some() {
            board.take_move(format!("{}-{}", mov.0.get_position(), mov.1.get(0).unwrap()));
            break;
        }
    }
    ...
    for x in 0..8usize {
        for y in 0..8usize {
            board_repr[x][y] = (if pieces.iter()
                .find(|piece| piece.get_position() == Position::new(x as u8 + 1, y as u8 + 1)).is_some() {
                Some(pieces.iter().find(|piece| piece.get_position() == Position::new(x as u8 + 1, y as u8 + 1)).unwrap().get_type())
            } else { None });
        }
    }
    ...
    for y in 0..8usize {
        for x in 0..8usize {
            print!("|{}", if board_repr.get(x).unwrap().get(7 - y).unwrap().is_some() {
                format!("{}", board_repr.get(x).unwrap().get(7 - y).unwrap().unwrap()) } else { " ".to_string() });
        }
        print!("|\n----------------\n");
    }
}